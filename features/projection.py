...
for path in img_path:
    ...
    row_non_zeros = np.count_nonzero(mask_edge, axis=1)
    column_non_zeros = np.count_nonzero(mask_edge, axis=0)
    ...
    cv2.imwrite(f'output/projection/{classes[i]}.png', mask_edge)

    cv2.imshow('Edge', mask_edge)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

    # Plot the histograms
    plt.figure()
    plt.plot(row_non_zeros, label='row_non_zeros')
    plt.plot(column_non_zeros, label='column_non_zeros')
    plt.xlim([0, 400])
    plt.legend()
